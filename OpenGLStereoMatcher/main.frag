...
{		
	...
	float min_cost = textureLod(TextureCost, vec3(1-UV.x, 0.5-UV.y/2, 0.0), average).r;	
	//colorOut = vec3(min_cost, min_cost, min_cost);
	float dx = 0.0;
	for(int layer=1; layer<int(layers); layer++)
	{		
		float cost = textureLod(TextureCost, vec3(1-UV.x, 0.5-UV.y/2, float(layer)), average).r;
